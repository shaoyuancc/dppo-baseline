...
from model.common.mpi_critic_wrapper import (
    DPPOViTCriticWrapper,
    MPICriticWrapper,
    MPICriticWrapperFresh,
)
...
class MPIPPODiffusion(nn.Module):
    ...
    def from_mpi_checkpoints(
        ...
        **kwargs,
        ...
        # Load MPI policy
        log.info("Loading MPI policy from %s", policy_checkpoint_path)
        mpi_policy = cls._load_mpi_policy(
            policy_checkpoint_path, policy_normalizer_path, device
        )
        ...
        if use_pretrained_critic:
            ...
            log.info("Loading pretrained MPI critic from %s", critic_checkpoint_path)
            mpi_value_network = cls._load_mpi_value_network(
                critic_checkpoint_path, critic_normalizer_path, device
            )
            ...
            log.info(
                "Creating fresh %s critic from config (will be trained from scratch)",
                critic_type.upper(),
            )
            ...
            if isinstance(critic, nn.Module):
                ...
                log.info("  Critic already instantiated: %s", type(critic).__name__)
                fresh_critic = critic
            else:
                # Critic is a config dict, instantiate it
                log.info("  Critic config: %s", critic)
            ...
            if critic_type == "vit":
                ...
                if hasattr(fresh_critic, "set_normalizer"):
                    ...
                    log.info(
                        "Set policy normalizer on fresh MPI critic for observation normalization"
                    )
                ...
                raise ValueError(
                    f"Unknown critic_type: {critic_type}. Must be 'mpi', 'vit', or 'mlp'."
                )

            n_critic_params = sum(p.numel() for p in fresh_critic.parameters())
            n_trainable = sum(
                p.numel() for p in fresh_critic.parameters() if p.requires_grad
            )
            log.info(
                "Fresh critic: %d total params, %d trainable",
                n_critic_params,
                n_trainable,
            )
    ...
    def _load_mpi_policy(checkpoint_path: str, normalizer_path: str, device: str):
        ...
        normalizer = torch.load(
            normalizer_path, map_location=device, weights_only=False
        )
        policy.set_normalizer(normalizer)
        log.info("Set normalizer with keys: %s", list(normalizer.params_dict.keys()))

        return policy.to(device)

    @staticmethod
    def _load_mpi_value_network(
        checkpoint_path: str, normalizer_path: str, device: str
    ):
        ...
        if "cfg" in checkpoint and hasattr(checkpoint["cfg"], "value_network"):
            ...
            try:
                from value_network.models.resnet18_tv_truck_2d_value_network import (
                    ResNet18TVTruck2dValueNetwork,
                )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.actor = MPIPolicyActorWrapper(
            ...
            obs_embedding_projector=getattr(
                mpi_policy, "obs_embedding_projector", None
            ),
        ).to(device)
        ...
        log.info("Actor: %d trainable parameters", n_params)
        ...
        log.info(
            "Using MPI noise scheduler: %s", type(self.mpi_noise_scheduler).__name__
        )
        ...
        if use_ddim:
            assert ddim_steps is not None, (
                "ddim_steps must be specified when use_ddim=True"
            )
        ...
        if use_ddim:
            ...
            log.info(
                "MPIPPODiffusion: DDIM with %d steps, finetuning last %d",
                ddim_steps,
                ft_denoising_steps,
            )
        else:
            log.info(
                "MPIPPODiffusion: DDPM with %d steps, finetuning last %d",
                denoising_steps,
                ft_denoising_steps,
            )
    ...
    def _setup_ddpm_params(self, denoising_steps: int):
        ...
        if (
            hasattr(self, "mpi_noise_scheduler")
            and self.mpi_noise_scheduler is not None
        ):
            ...
            log.info(
                "Using DDPM parameters from MPI noise scheduler (squaredcos_cap_v2)"
            )
        else:
    ...
    def _setup_ddim_params(
        self, denoising_steps: int, ddim_steps: int, ddim_discretize: str
    ):
        """
        ...
        log.info(
            "DDIM schedule: %d steps, timesteps=%s", ddim_steps, self.ddim_t.tolist()
        )
    ...
    def forward(
        ...
        if (
            not self.use_ddim
            and self.use_mpi_scheduler
            and hasattr(self, "mpi_noise_scheduler")
        ):
    ...
    def step(self):
        ...
        if (
            ...
            for param in self.actor.parameters():
                param.requires_grad = False
            log.info(
                "Finished annealing fine-tuning denoising steps to %d",
